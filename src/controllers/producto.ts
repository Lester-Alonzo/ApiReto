...
export async function MiddlewareProduct(req:Express.RequestS, res:Response, next:NextFunction):Promise<void> {
    try {
...
export async function CraerProducto(req: Express.RequestS, res: Response):Promise<void> {
  ...
  try {
    ...
    const fotoUrl = await UpImage(Buffer.from(req.file?.buffer as Buffer).toString("base64"))
    const [resultado] = await sequelize.query(
...
export async function EliminarProducto(req:Express.RequestS, res:Response) {
  ...
  try {
    let result = await sequelize.query(`EXEC InactivarProductor :idProducto, :idEstado`, {
      replacements:{
        idProducto: id,
        idEstado:estado
      }
