...
export async function MiddlewareCP(req:Express.RequestS, res:Response, next:NextFunction):Promise<void> {
    try {
...
export async function CrearCategoria(req:Express.RequestS, res:Response) {
    ...
    try {
        let rs = await sequelize.query(`EXEC CrearCategorias :usuarios_idusuarios, :nombre, :estados_idestados`, {
            replacements:{
...
export async function EditarCategoria(req:Request, res:Response) {
    ...
    const {nombre, estado} = req.body
    try {
        await sequelize.query(`EXEC ActualizarCategoria :idCategoriaProductos, :nombre, :estados_idestados`, {
            replacements:{
